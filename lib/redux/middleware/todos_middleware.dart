...
//
// import 'package:ifdf/models/todos/todo.dart';
...
//     next(TodosSaveAction());
//   }),
//   TypedMiddleware<AppState, TodoToggleCompletionAction>((store, action, next) async {
//     final todosState = store.state.todosState;
//     final todos = todosState.todos
...
//     final todoJsons = sharedPreferences.getStringList(_todosJsonSharedPreferencesKey) ?? [];
//     final todos = todoJsons
//         .asMap()
//         .map((key, value) => MapEntry(key, Todo.fromJson(json.decode(value)))).values
//         .toList(growable: false);
//     next(TodosSetAction(nextId: todos.fold<int>(0, (previousValue, todo) => math.max(todo.id, previousValue)) + 1, todos: todos));
//   }),
//   TypedMiddleware<AppState, TodosSaveAction>((store, action, next) async {
//     final sharedPreferences = await SharedPreferences.getInstance();
//     final todos = store.state.todosState.todos.map((todo) => json.encode(todo.toJson())).toList(growable: false);
//     await sharedPreferences.setStringList(_todosJsonSharedPreferencesKey, todos);
//   })
// ];
//
